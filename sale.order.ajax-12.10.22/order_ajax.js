...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
      ...
      //sort properties
      this.result.ORDER_PROP.properties.sort((a, b) => {
        return 1 * a.SORT - 1 * b.SORT;
      });
		...
		locationsCompletion: function()
		{
			...
			//if (this.activeSectionId !== this.regionBlockNode.id)
				this.editFadeRegionContent(this.regionBlockNode.querySelector('.bx-soa-section-title-content'));

			if (this.activeSectionId != this.propsBlockNode.id)
				this.editFadePropsContent(this.propsBlockNode.querySelector('.bx-soa-section-content'));
      
      //show all blocks except region
      ...
      /*this.reachGoal('order');
		 ...
		 */
		show: function(node, showByClickFlag)
		{
			if (!node || !node.id/* || this.activeSectionId == node.id*/)
				return;
      
      if (this.firstLoad && !showByClickFlag && node.id === this.regionBlockNode.id && this.getLocationString(this.regionHiddenBlockNode) !== BX.message('SOA_NOT_SPECIFIED') ||
          !this.firstLoad && !showByClickFlag && node.id === this.regionBlockNode.id && !this.regionBlockNode.querySelector('.bx-soa-editstep').classList.contains('hidden')) {
        //first load with the locations defined
        //or location defined and the regio block is hidden (show method invoked after delivery or paysystem change)
        return;
      }
      
      node.querySelector('.bx-soa-editstep').classList.add('hidden');
      ...
      event.target.classList.add('hidden');
		...
		editSection: function(section)
		{
			...
			var active = false,//section.id == this.activeSectionId,
				titleNode = section.querySelector('.bx-soa-section-title-container'),
		...
		editActiveRegionBlock: function(activeNodeMode)
		{
			...
			if (activeNodeMode && this.initialized.region)
			{
		...
		editFadeRegionContent: function(node)
		{
			...
			if (locationProperty && locationString.length)
				addedHtml += BX.util.htmlspecialchars(locationString);
		...
		createPaySystemItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				...
				title, label, description, itemNode;
				...
				children: [
					BX.create('INPUT', {
						props: {
							...
							name: 'PAY_SYSTEM_ID',
							type: 'checkbox',
							className: 'bx-soa-pp-company-checkbox with-gap',
							value: paySystemId,
							checked: checked
						}
					}),
      ...
      description = BX.create('DIV', {props: {className: 'bx-soa-pp-company-description'}, html: item.DESCRIPTION});

			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company'},
				children: [label, title, description],
				events: {
		...
		createDeliveryItem: function(item) {
          ...
          BX.create('SPAN')
				...
				label, titleBlock, title, description, price, itemNode;

			label = BX.create('DIV', {
				props: {
					className: 'bx-soa-pp-company-graf-container'
					+ (item.CALCULATE_ERRORS ? ' bx-bd-waring' : '')},
				children: labelNodes
			});
      ...
      price = BX.create('DIV', {
        props: {className: 'bx-soa-pp-company-price'},
        html: item.PRICE_FORMATED
      });

			titleBlock = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company-title-block'},
				children: [title, price]
			});

			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company'},
				children: [label, titleBlock, description],
		...
		activatePickUp: function(deliveryName)
		{
			...
			this.pickUpBlockNode.querySelector('h2.bx-soa-section-title').innerHTML = BX.util.htmlspecialchars(deliveryName);
		...
		insertStringProperty: function(property, propsItemNode, disabled)
		{
			...
			else
			{
				...
				this.alterProperty(property.getSettings(), propContainer);
				this.bindValidation(property.getId(), propContainer);
        /*if (1 * property.getId() === 22 && window.IMask) {
          IMask(property.getParentNode().querySelector('input'), { mask: '+{7} (000) 000 00 00' });
        }*/
        
        if (property.getValue() !== '') {
          propsItemNode.classList.add('form-group--active');
        }
        
        property.addEvent('focus', () => {
          propsItemNode.classList.add('form-group--active');
        });

        property.addEvent('blur', () => {
          if (property.getValue() !== '') {
            propsItemNode.classList.add('form-group--active');
          } else {
            propsItemNode.classList.remove('form-group--active');
          }
        });
			}
		},
		...
		editPropsComment: function(propsNode)
		{
			...
			propsCommentContainer = BX.create('DIV', {props: {className: 'bx-soa-customer-comment'/*'col-sm-12'*/}});
			label = BX.create('LABEL', {
      ...
      input.addEventListener('blur', () => {
		...
		editTotalBlock: function()
		{
			...
			{
        let name = BX.message('SOA_SUM_DELIVERY');
        this.result.DELIVERY.forEach(deliveryItem => {
          if (deliveryItem.CHECKED === 'Y' && deliveryItem.ID === '3') {
            name = deliveryItem.NAME;
          }
        });
				this.totalInfoBlockNode.appendChild(this.createTotalUnit(name, deliveryValue, params));
			}
		...
		totalBlockScrollCheck: function()
		{
			...
			var scrollTop = BX.GetWindowScrollPos().scrollTop,
        headerHeight = document.querySelector('.b-header').offsetHeight + 20,
				ghostTop = BX.pos(this.totalGhostBlockNode).top - headerHeight,
				ghostBottom = BX.pos(this.orderBlockNode).bottom - headerHeight,
				width;
			...
			if (scrollTop > ghostTop && !BX.hasClass(this.totalInfoBlockNode, 'bx-soa-cart-total-fixed'))
			{
				...
				this.totalInfoBlockNode.style.top = headerHeight + 'px';
			}
			else if (scrollTop < ghostTop && BX.hasClass(this.totalInfoBlockNode, 'bx-soa-cart-total-fixed'))
			{
				...
				this.totalInfoBlockNode.style.top = '';
			}
		},
